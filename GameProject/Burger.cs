    ...
    public class Burger
    {
        ...
        public void Update(GameTime gameTime, /*MouseState mouse*/ KeyboardState keyboard)
        {
            ...
            if (health > 0)
            {
                ...
                //get burger to follow the maouse
            ...
            if (!canShoot)
            {
                ...
                if (elapsedCooldownMilliseconds >= GameConstants.BurgerTotalCooldownMilliseconds ||
                   /* mouse.LeftButton == ButtonState.Released*/ keyboard.IsKeyUp(Keys.Space))
                {
                    canShoot = true;
            ...
            if (health> 0 && 
                /*mouse.LeftButton == ButtonState.Pressed*/ keyboard.IsKeyDown(Keys.Space) &&
                canShoot)
            {
                    canShoot = false;
                    int y = drawRectangle.Y - GameConstants.FrenchFriesProjectileOffset;
                    Projectile newProjectile = new Projectile(ProjectileType.FrenchFries, 
                        Game1.GetProjectileSprite(0), drawRectangle.X,
                        y, GameConstants.FrenchFriesProjectileSpeed);
        ...
        {
            //Drawing the bear itself method
